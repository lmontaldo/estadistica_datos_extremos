...
Una vez que se tiene la serie de datos "indicador", se filtran las filas del conjunto de datos de forma tal que los valores del $IVP>=0$. Con esto se obtienen los datos del estudio que serán objeto de un análisis con el método POT.
...
En @notas se señala la importancia del \textit{trade-off} sobre el umbral $u$. Por un lado es preciso que sea grande, pero no lo suficiente como para que la submuestra resultante tenga pocos datos, dado que se podría perder confiabilidad en los resultados. Para este estudio se elige $u=0.04$
...
ggplot(indicador_df, aes(x = Date, y = indicador)) +
  ...
  ggtitle("Fijo un umbral para los datos del estudio") +
  xlab("Fecha") +
  ylab("Valores del Indicador") +
  scale_x_date(limits = date_range) +
