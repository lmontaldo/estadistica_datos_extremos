...
Los datos se descargan mediante una consulta a la API de yahoo\!finance empleando el módulo de python \texttt{yfinance}.  Para este estudio, se toman los valores al cierre de cada día para la serie temporal de $S\&\;500$ desde la fecha 03/01/1928 hasta 08/12/2023. Por lo tanto, se cuenta con una serie temporal de frecuencia diaria con 24.195 filas.
 
Tomando la serie de datos diaria se busca crear un indicador de una posible crisis bursátil denominado Índice de Variación de Precios (IVP). Como variable de referencia de toma la relación de precios al cierre de ayer sobre la de hoy.
...
A partir de la columna "Close", que indica el valor del índice $S\&P\;500$ al cierre de cada día, se crea la columna "relacion" a la que se le resta el valor 1 y sea genera la columna "indicador" para eliminar esa constante determinística en los datos.
...
En la Figura 1 se muestra la evolución histórica del valor al cierre de cada día del indicador $S\&P\;500$, desde la fecha 03/01/1928 hasta 08/12/2023.


```{r fig1, fig.cap="Valores históricos diarios de cierre del índice S&P500 (03/01/1928-08/12/2023)", echo=FALSE}
# Your ggplot code
...
Una vez que se tiene la serie de datos "indicador", se filtran las filas del conjunto de datos de forma tal que los valores del $IVP>=0$ con lo que se obtiene como resultado un conjunto de datos con 11.529 filas. Con esto se obtienen los datos del estudio que serán objeto de un análisis con el método POT, como se muestra en la Figura 2.
...
```
```{r include=FALSE}
cat("Cantidad de filas del conjunto de datos: ", dim(indicador_df)[1])
```



```{r fig2, fig.cap=" Conjuto de datos del estudio (IVP con valores iguales o mayores a cero)", echo=FALSE, fig.lp="fig:figure2"}
...
En la próxima sección se desarrolla el marco teórico que sirve de base para la entrategia empírica posterior y para el análisis de sus resultados.
...
Si los datos no necesariamente son independientes pero la estructura de distribuciones es siempre la misma, se habla de datos estacionarios (o procesos estacionarios). Siguiendo a @enders, se dice que un proceso estocástico $z_{t}$ con media y varianza finitas es estacionario en covarianza para todo $t$ y $t-s$ cuando
...
## Análisis de raíz unitaria y estacionariedad 

```{r include=FALSE}
Obs=indicador_df
rownames(Obs) <- Obs$Date
Obs <- Obs[, -1]
head(Obs)
```
...
## Implementación del método POT

En @notas_curso se señala la importancia del \textit{trade-off} sobre el umbral $u$. Por un lado es necesario que sea grande, pero no lo suficiente como para que la submuestra resultante tenga pocos datos, dado que se podría perder confiabilidad en los resultados. Para este estudio se elige $u=0.025$ como se observa en la Figura 3.
...
# Paso 1
umbral<-0.025
exceso<-which(Obs > umbral) #  valores que exceden el umbral
# Paso 2
p<- 1 - (length(exceso) / length(Obs)) 
```

```{r echo=FALSE}
cat("Probabilidad de quedar bajo el umbral p=F(u): ", round(p,3))
cat("Cantidad de datos del estudio:", length(Obs), "\n")
cat("Cantidad de datos que exceden el umbral u=0.025:" ,length(exceso),  "\n")
cat("Proporción de datos en exceso respecto al total del estudio:", round((length(exceso)/length(Obs)*100),2), "%")
```
Para aportar información adicional para la selección del umbral, también calculo el máximo valor de los datos, el valor medio y la mediana como se muestra en la Tabla \ref{tabla_1}.
...
# Calculate the maximum, mean, and median of the "indicador" column
max_value <- max(indicador_df$indicador)
mean_value <- mean(indicador_df$indicador)
median_value <- median(indicador_df$indicador)

# Create a data frame with the summary statistics
summary_df <- data.frame(
  medidas_estadísticas = c("Máximo", "Media", "Mediana"),
  Valor_Indicador = c(max_value, mean_value, median_value)
)

# Convert the data frame to an xtable object
summary_table <- xtable(summary_df, caption="Valores máximo, medio y mediana de los datos", digits = 3, label="tabla_1")

# Print the table
print(summary_table, include.rownames = FALSE)
```
\begin{table}[ht]
\centering
\begin{tabular}{lr}
...
```{r  fig3, fig.cap="Umbral igual a 0.025 para los datos del estudio" ,echo=FALSE, fig.lp="figure3"}
ggplot(indicador_df, aes(x = Date, y = indicador)) +
  geom_line() +
  geom_hline(yintercept = umbral, color = "red") +  # Add horizontal line at y = 0.025
  xlab("Fecha") +
  ylab("Valores del Indicador") +
  scale_x_date(limits = date_range) +
  scale_y_continuous(breaks = seq(0, ceiling(max(df$indicador)), by = 0.05)) +
  theme(
    axis.title.x = element_text(margin = margin(t = 20, b = 40)),
    axis.title.y = element_text(margin = margin(r = 20, l = 40)),
    plot.title = element_text(size = 11, hjust = 0.5),  # Center the plot title
...
```{r}
# Paso 3: Esto es lo que llamamos evento
y<- Obs[exceso] - umbral 
head(y)
```
